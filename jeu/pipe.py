...
class Pipe:
    def __init__(self, screen_width, gap, screen_height, top_pipe_height, image_top=image_tuyau_haut, image_bottom=image_tuyau_bas):
        ...
        self.image_top = pygame.transform.scale(
            image_top, (90, self.top_height))
        self.image_bottom = pygame.transform.scale(
            image_bottom, (90, self.bottom_height))
        self.width = self.image_top.get_width()

        # Définition de la hitbox
        self.top_rect = pygame.Rect(
            self.x, self.y, self.width, self.top_height)
        self.bottom_rect = pygame.Rect(
            self.x, self.top_height+self.gap, self.width, self.bottom_height)
